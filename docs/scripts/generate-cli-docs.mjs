...
function processMinor({ minor, patch, unlisted, rawDir, destDir, position }) {
  ...
  fs.writeFileSync(
    path.join(staging, 'commands.json'),
...
function main() {
  ...
  // Keep the React app's copy in sync (webpack cannot always import outside site/).
  const dataDir = path.join(DOCS_ROOT, 'site', 'src', 'data');
  fs.mkdirSync(dataDir, { recursive: true });
  fs.writeFileSync(
    path.join(dataDir, 'cli-versions.json'),
