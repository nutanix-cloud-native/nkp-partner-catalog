---
title: nkp
sidebar_label: nkp
sidebar_position: 0
cli_generated: true
unlisted: true
hide_table_of_contents: true
displayed_sidebar: tutorialSidebar
nkp_minor: 2.18
nkp_patch: 2.18.0
nkp_command_id: nkp
nkp_command_kind: parent
---

<div class="cli-doc-ref" aria-hidden="true"></div>

### Available commands

* [nkp attach](nkp_attach.md) — Attach one of \[cluster\]
* [nkp check](nkp_check.md) — Check, one of \[cluster\]
* [nkp cluster](nkp_cluster.md) — Get cluster information
* [nkp completion](nkp_completion.md) — Generate the autocompletion script for the specified shell
* [nkp config](nkp_config.md) — Manage Kommander configuration
* [nkp create](nkp_create.md) — Create one of \[appdeployment, bootstrap, bundle, capi-components, catalog-application, catalog-bundle, cluster, image, image-bundle, metadata, nodepool, package-bundle, project, workspace\]
* [nkp delete](nkp_delete.md) — Delete one of \[bootstrap (cluster), capi-components, cluster, nodepool\]
* [nkp describe](nkp_describe.md) — Describe one of \[cluster\]
* [nkp detach](nkp_detach.md) — Detach one of \[cluster\]
* [nkp diagnose](nkp_diagnose.md) — Generate a support bundle
* [nkp edit](nkp_edit.md) — Edit a resource on the server
* [nkp experimental](nkp_experimental.md) — Experimental one of \[catalog, gitops, rotate\]
* [nkp generate](nkp_generate.md) — Generate one of \[catalog-repository\]
* [nkp get](nkp_get.md) — Get one of \[appdeployments, clusters, dashboard, kubeconfig, nodepools, projects, workspaces\]
* [nkp import](nkp_import.md) — Import images from an image bundle into Containerd
* [nkp install](nkp_install.md) — Install one of \[kommander\]
* [nkp move](nkp_move.md) — Move one of \[capi-resources\]
* [nkp open](nkp_open.md) — Open one of \[dashboard\]
* [nkp push](nkp_push.md) — Push one of \[bundle, image-archive, image-bundle\]
* [nkp scale](nkp_scale.md) — Scale one of \[nodepool\]
* [nkp serve](nkp_serve.md) — Serve image or Helm chart bundles from an OCI registry
* [nkp update](nkp_update.md) — Update one of \[bootstrap (cluster), controlplane, nodepool\]
* [nkp upgrade](nkp_upgrade.md) — Upgrade one of \[addons, capi-components, catalogapp, cluster, kommander, workspace\]
* [nkp upload](nkp_upload.md) — Upload image artifacts to remote hosts
* [nkp validate](nkp_validate.md) — Validate one of \[catalog-repository\]
* [nkp version](nkp_version.md) — Print version information
