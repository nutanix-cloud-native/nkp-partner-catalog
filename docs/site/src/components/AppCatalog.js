...
const { matchesNkpVersion, DEFAULT_NKP_VERSIONS, gaNkpVersions } = nkpVersion;
...
function nkpLabel(app) {
  return (app.nkpRange && app.nkpRange.label) || 'NKP 2.16+';
}
...
function appMatches(app, catalog, filters, except) {
  ...
  if (except !== 'nkp' && !matchesNkpVersion(app.nkpRange, filters.nkp)) return false;
...
export default function AppCatalog() {
  ...
  const facetRail = (
    <aside className="cat-facets" aria-label="Catalog filters">
      <div className="cat-facet">
        <h3 className="cat-facet-title">NKP version</h3>
        <label className="cat-sr-only" htmlFor="cat-nkp-select">NKP version</label>
        <select
          id="cat-nkp-select"
          className="cat-select"
          value={effectiveFilters.nkp}
          onChange={e => updateFilters({ nkp: e.target.value })}
        >
          <option value="all">All versions</option>
          {[...nkpVersions].slice().reverse().map(v => (
